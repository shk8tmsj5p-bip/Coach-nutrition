...
import { isFocusTab } from "@/lib/tabs";
import { cn } from "@/lib/utils";
import type { ProfileId } from "@/lib/types";

const FOCUS: { id: ProfileId; label: string }[] = [
...
export function ProfileToggle() {
  ...
  const showFocus = isFocusTab(pathname);
  ...
  return (
    <header className="sticky top-0 z-30 bg-health-bg/85 px-4 pb-3 pt-[calc(var(--safe-top)+10px)] backdrop-blur-xl">
      <div className="flex items-center gap-2">
        ...
        <div className={cn("flex shrink-0 items-center gap-2", !showFocus && "ml-auto")}>
          <WeatherChip />
          <button
            type="button"
